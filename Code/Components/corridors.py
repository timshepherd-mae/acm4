...
def build_corridor_products(
    ...
    *,
    ...
    Returns:
        multipointZ : MultiPointZ (boundary vertices with Z sampled from DTM at nearest centerline XY)
    ...
    # -------- Offsets (QGIS 3.44 requires full signature) --------
    half_w = width / 2.0
    segments = 2                              # increase if you want smoother rounded corners
    ...
    def _sample_filtered(xy: QgsPointXY) -> Optional[float]:
        ...
        if val is None:
            return None
        # Treat NoData as None (e.g., many DEMs use 0.0 as NoData)
        if nodata_val is not None:
            try:
                # If nodata is nan, compare safely
    ...
    # -------- Sample Z for each boundary vertex (using nearest centerline XY) --------
    z_points: List[QgsPoint] = []

    for pt in boundary_ring[:-1]:  # exclude duplicated closing vertex
        # Nearest point on centerline (project CRS)
        ...
        # Guard: only sample if inside raster extent
        z_val: Optional[float] = None
        if dtm_raster.extent().contains(sample_xy_for_raster):
            z_val = _sample_filtered(sample_xy_for_raster)

        # Fallback: try the boundary vertex XY (transformed) if nearest failed or was outside
        if z_val is None:
            alt_xy = pt
            if xform:
                try:
        ...
        # Final fallback to keep pipeline robust (or raise if you prefer strictness)
        if z_val is None:
            z_val = 0.0
