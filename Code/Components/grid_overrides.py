...
def do(
    ...
    *,
    ...
    gpkg_points    = _temp_vector("acm4_rotgrid_points")
    gpkg_breakline = _temp_vector("acm4_rotgrid_lines")
    gpkg_polygons  = _temp_vector("acm4_rotgrid_polys")

    # ---- Create layers inside the GPKGs (IN PROJECT CRS)
    points_src = _create_gpkg_layer(
        gpkg_points, points_layer_name, qgc.QgsWkbTypes.PointZ, work_crs,
        _schema_points()
    )
    breaklines_src = _create_gpkg_layer(
        gpkg_breakline, breaklines_layer_name, qgc.QgsWkbTypes.LineString, work_crs,
        _schema_breaklines()
    )
    polygons_src = _create_gpkg_layer(
        gpkg_polygons, polygons_layer_name, qgc.QgsWkbTypes.Polygon, work_crs,
        _schema_polygons()
    )

    # ---- Open layers and start editing
    ptsL  = QgsVectorLayer(points_src,    "points_out",    "ogr")
    blL   = QgsVectorLayer(breaklines_src,"breaklines_out","ogr")
    polyL = QgsVectorLayer(polygons_src,  "polygons_out",  "ogr")
    if not all([ptsL.isValid(), blL.isValid(), polyL.isValid()]):
        raise RuntimeError(f"Failed to open one or more outputs:\n{points_src}\n{breaklines_src}\n{polygons_src}")

    # Validate WKB + fields
    _assert_layer_is(ptsL,  "PointZ")
    _assert_layer_is(blL,   "LineString")
    _assert_layer_is(polyL, "Polygon")
    _assert_fields(ptsL,  ("parent_fid", "type", "Z", "ZMethod"))
    _assert_fields(blL,   ("parent_fid", "type"))
    _assert_fields(polyL, ("parent_fid", "type"))
    ...
    # Capabilities info
    for L, lbl in ((ptsL,"points"), (blL,"breaklines"), (polyL,"polygons")):
        caps = L.dataProvider().capabilities()
        can_add = bool(caps & QgsVectorDataProvider.AddFeatures)
        QgsMessageLog.logMessage(
            f"[grid_overrides] {lbl}: editable={L.isEditable()} can_add={can_add} wkb={qgc.QgsWkbTypes.displayString(L.wkbType())}",
    ...
    if debug or add_to_canvas:
        try:
            add_vector(points_src,    "6-GridOver - Points 3d")
            add_vector(breaklines_src,"6-GridOver - Breaklines")
            add_vector(polygons_src,  "6-GridOver - PolygonMask")
        except Exception:
            pass
    ...
    return points_src, breaklines_src, polygons_src
