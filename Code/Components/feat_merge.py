...
def do(
    ...
    path_vector_poly_over_buffer = _temp_vector("acm4_poly_over_buffer")
    path_vector_points_slope_culled = _temp_vector("acm4_points_slope_culled")
    path_vector_points_over_merged = _temp_vector("acm4_points_over_merged")
    path_vector_poly_corr_buffer = _temp_vector("acm4_poly_corr_buffer")
    path_vector_points_over_culled = _temp_vector("acm4_points_over_culled")
    ...
    # cutout override buffered mask_over
    ...
    # merge slope and override points

    result = processing.run("native:mergevectorlayers",
        {
            "LAYERS": [path_vector_points_slope_culled, point_over],
            "OUTPUT": path_vector_points_over_merged
        }
    )
    ...
    result = processing.run("native:difference",
        {
            "INPUT": path_vector_points_over_merged,
    ...
    result = processing.run("native:mergevectorlayers",
        {
            "LAYERS": [path_vector_points_over_culled, point_corr],
    ...
    result = processing.run("native:mergevectorlayers",
        {
            "LAYERS": [break_over, break_corr, path_vector_breaks_extent_explode],
    ...
    if debug:
        ...
        _ = add_vector(path_vector_points_slope_merged, "6-FeatureMerge - Slope Points Merged")
        _ = add_vector(path_vector_poly_over_buffer, "6-FeatureMerge - Override Mask")
        _ = add_vector(path_vector_points_slope_culled, "6-FeatureMerge - Slope Points With Override Cutout")
        _ = add_vector(path_vector_points_over_merged, "6-FeatureMerge - Override Points Merged")
        _ = add_vector(path_vector_poly_corr_buffer, "6-FeatureMerge - Corridor Mask")
        _ = add_vector(path_vector_points_over_culled, "6-FeatureMerge - Slope/Override Points With Corridor Cutout")
        _ = add_vector(path_vector_points_corr_merged, "6-FeatureMerge - Corridor Points Merged")
        _ = add_vector(path_vector_poly_extent_buffer, "6-FeatureMerge - Extent Mask")
        _ = add_vector(path_vector_points_extent_culled, "6-FeatureMerge - Points Trimmed To Extents")
        _ = add_vector(path_vector_points_extent_perimeter_Z, "6-FeatureMerge - Perimeter Points")

        _ = add_vector(path_vector_points_ground_product, "6-FeatureMerge - Corridor Points Merged (final product)")
        _ = add_vector(path_vector_breaks_ground_product, "6-FeatureMerge - Breaklines Merged (final product)")
